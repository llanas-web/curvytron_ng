...
export class RoomRepository extends EventEmitter {
  ...
  kickPlayer(player: Player, callback: (arg: any) => any) {
    this.client.addEvent("player:kick", { player: player.id }, (result) => {
  ...
  setColor(player: Player, color: string, callback: (arg: any) => any) {
    this.client.addEvent(
      ...
      (result) => {
        if (!result.success) {
          console.error(
            "Could not set color %s for player %s",
            player.color,
            player.name
          );
        }
   ...
   */
  @boundMethod
  onClientAdd({ client }: { client: { id: number; active: boolean } }) {
    this.clients.add(new Client(client.id, client.active));
  }

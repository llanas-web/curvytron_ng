...
export class RoomComponent extends EventEmitter implements OnInit {
  ...
  setProfileControls(player: {
    controls: { loadMapping: (arg0: any) => void }[];
  }) {
  ...
  removePlayer(player: { local: any; name: any }) {
    ...
    this.repository.removePlayer(player, (result: { success: any }) => {
      if (!result.success) {
  ...
  setColor(player: Player): any {
    ...
    this.repository.setColor(player, player.color, (result) => {
  ...
  setName(player: Player): any {
    ...
    this.repository.setName(player, player.name, (result) => {
      if (!result.success) {
        const error =
          typeof result.error !== "undefined" ? result.error : "Unknown error";
  ...
  setReady(player: { local: any; id: Player; name: any }): any {
    ...
    this.repository.setReady(player.id, (result) => {
      if (!result.success) {
  ...
  @boundMethod
  start(e: any) {
    const passwordQuery = this.room.config.open

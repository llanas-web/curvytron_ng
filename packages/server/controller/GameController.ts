...
export class GameController {
  ...
  constructor(game: Game) {
    ...
    this.callbacks = {
      ...
      onSpeeding(data) {
        controller.onSpeeding(this, data);
      },
    };
  ...
  loadGame() {
    ...
    for (
      let i = this.game.room.controller.clients.items.length - 1;
      i >= 0;
      i--
    ) {
  ...
  attachEvents(client: ServerSocketClient) {
    ...
    if (!client.players.isEmpty()) {
      client.on("player:move", (arg: any) => this.onMove(client, arg));
      client.on("player:speeding", (arg: any) => this.onSpeeding(client, arg));
    }
  ...
  detachEvents(client: ServerSocketClient) {
    let avatar;
    client.removeListener("ready", this.onReady);
    if (!client.players.isEmpty()) {
  ...
  attachSpectator(client: ServerSocketClient) {
        ...
        if (properties.hasOwnProperty(property)) {
          events.push([
            "property",
            {
              avatar: avatar.id,
              property,
              value: avatar[properties[property]],
            },
          ]);
        }
      }
    ...
    if (this.game.inRound) {
      ...
      this.socketGroup.addEvent(
        "round:end",
        this.game.roundWinner ? this.game.roundWinner.id : null
      );
    }
  ...
  onReady(client: ServerSocketClient) {
    if (this.game.started) {
      ...
      for (
        let avatar: Avatar, i = client.players.items.length - 1;
        i >= 0;
        i--
      ) {
        avatar = client.players.items[i].getAvatar();
  ...
  onMove(
    ...
    { avatar, move }: { avatar: string; move: number }
  ) {
  ...
  onSpeeding(
    client: ServerSocketClient,
    { avatar, speeding }: { avatar: string; speeding: number }
  ) {
    const player = client.players.getById(avatar);
    if (player && player.avatar) {
  ...
  onProperty({
    ...
    this.socketGroup.addEvent("property", [avatar.id, property, value]);
  }

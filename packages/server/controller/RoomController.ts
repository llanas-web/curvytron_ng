...
export class RoomController extends EventEmitter {
  ...
  attach(client: ServerSocketClient, callback: (arg: any) => any) {
    if (this.clients.add(client)) {
      ...
      callback({
        ...
        votes: this.kickManager.votes.map((vote: any) => client.serialize())
          .items,
      });
      ...
      callback({
        success: false,
        error: "Client " + client.id + " already in the room.",
      });
    }
    this.checkIntegrity();
  }
  ...
  nominateRoomMaster() {
    ...
    const roomMaster = this.clients.match(
      (client: ServerSocketClient) => client.active && client.isPlaying()
    );
  ...
  setRoomMaster(client: ServerSocketClient) {
    if (!this.roomMaster && client) {
      ...
      this.roomMaster.on(
        ...
        this.callbacks.onConfigMaxScore
      );
      this.roomMaster.on(
        "room:config:variable",
        this.callbacks.onConfigVariable
      );
  ...
  checkIntegrity() {
    for (
      let player: Player, i = this.room.players.items.length - 1;
      i >= 0;
      i--
    ) {
  ...
  onColor(client: ServerSocketClient, data: any, callback: (arg: any) => any) {
    ...
    if (player.setColor(color)) {
      ...
      this.socketGroup.addEvent("player:color", {
        player: player.id,
        color: player.color,
      });
    } else {
  ...
  onName(client: ServerSocketClient, data: any, callback: (arg: any) => any) {
    ...
    this.socketGroup.addEvent("player:name", {
      player: player.id,
      name: player.name,
    });
  }
  ...
  onReady(client: ServerSocketClient, data: any, callback: (arg: any) => any) {
    ...
    if (player) {
      ...
      this.socketGroup.addEvent("player:ready", {
        player: player.id,
        ready: player.ready,
      });
      ...
      callback({
        success: false,
        error: 'Player with id "' + data.player + '" not found',
      });
    }
  }
  ...
  onConfigOpen(
    ...
    const success =
      this.isRoomMaster(client) && this.room.config.setOpen(data.open);
    callback({
      success,
  ...
  onConfigMaxScore(
    ...
    const success =
      this.isRoomMaster(client) && this.room.config.setMaxScore(data.maxScore);
    callback({ success, maxScore: this.room.config.maxScore });
    if (success) {
      this.socketGroup.addEvent("room:config:max-score", {
        maxScore: this.room.config.maxScore,
      });
    }
  }
  ...
  onConfigBonus(
    ...
    const success =
      this.isRoomMaster(client) && this.room.config.toggleBonus(data.bonus);
  ...
  removeRoomMaster() {
    if (this.roomMaster) {
      ...
      this.roomMaster.removeListener(
        "room:config:open",
        this.callbacks.onConfigOpen
      );
      this.roomMaster.removeListener(
        "room:config:max-score",
        this.callbacks.onConfigMaxScore

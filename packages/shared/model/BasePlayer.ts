...
export abstract class BasePlayer extends EventEmitter {
  ...
  static validateColor(color: string, yiq?: any /* NFC */): boolean {
    ...
    const matches = color.match(
      new RegExp("^#([a-fA-F0-9]{2})([a-fA-F0-9]{2})([a-fA-F0-9]{2})$")
    );
    if (matches && yiq) {
      const ratio =
  ...
  constructor(clientId: string, name: string, color: string, ready = false) {
    ...
    this.name = name;
    this.color =
      typeof color !== "undefined" && BasePlayer.validateColor(color)
        ? color
        : BasePlayer.getRandomColor();
  }
  ...
  toggleReady(toggle: boolean) {
    this.ready =
      typeof toggle !== "undefined" ? (toggle ? true : false) : !this.ready;
  }
